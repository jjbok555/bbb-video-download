...
const combinedSlidesAndDeskshares = async (slides, deskshares, config, duration) => {
    ...
    deskshares.parts.forEach((part, index) => {
        ...
        childProcess.execSync(`ffmpeg -hide_banner -loglevel error -threads 4 -i ${presentationIn} -i ${resizedDesksharesVideo} -filter_complex "[0][1]overlay=x=0:y=0:enable='between(t,${part.start},${part.end})'[out]" -map [out] -c:a copy -c:v libx264 -preset ultrafast ${presentationTmp}`)
